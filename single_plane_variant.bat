...
REM ============================================================
REM Single-plane variant of run_process_spim_and_compress_batch.bat
REM
REM KEEP IN SYNC with the main script: any change to the main
REM workflow (scanning, prompts, pre-flights, logging, compression)
REM must be mirrored here, EXCEPT for these intentional differences:
REM   1) Registration uses bin\Process_SPIM_SinglePlane_v1.0.exe
REM   2) Registration stdin is exactly 7 lines, in order:
REM      source dir, target dir, digits of filenames, min time point,
REM      max time point, reference time point, camera number
REM      (dirs passed with trailing backslash, like stack2h5_v2.exe)
REM   3) No server selection prompt (the exe takes no server_ind /
REM      extRef / rdir inputs)
REM   4) Extra verification: ch0_cam<CAM>.xml should contain
REM      <info hs_single_plane="T" /> (warn-only, never skips)
REM ============================================================

REM Binaries (exes + DLLs) live in this subfolder so the script
REM directory stays clean (only .bat files + README at the root).
set "BIN_DIR=bin"

echo.
echo ================================================
echo   SPIM Registration + h5 Compression - Batch
echo   (Single-Plane Variant)
echo ================================================
echo.
...
REM ============================================================
if not exist "%BIN_DIR%\Process_SPIM_SinglePlane_v1.0.exe" (
    echo [ERROR] Process_SPIM_SinglePlane_v1.0.exe not found in: %~dp0%BIN_DIR%
    echo Place Process_SPIM_SinglePlane_v1.0.exe and its DLLs in the %BIN_DIR% subfolder.
    pause
    exit /b 1
)
...
REM ---- Single-plane verification: ch0_cam<CAM>.xml should contain ----
REM ---- <info hs_single_plane="T" />  (warn-only, never skips)     ----
REM States: T / F / no line / no xml
set "PV_XML=!PV_SRC!\ch0_cam!PV_CAM!.xml"
set "PV_SP=no xml"
if exist "!PV_XML!" (
    set "PV_SP=no line"
    find "hs_single_plane=" < "!PV_XML!" >nul
    if not errorlevel 1 (
        set "PV_SP=F"
        find "hs_single_plane=""T""" < "!PV_XML!" >nul
        if not errorlevel 1 set "PV_SP=T"
    )
)
...
if not "!PV_SP!"=="T" (
    echo  [WARNING] hs_single_plane is "!PV_SP!" in ch0_cam!PV_CAM!.xml
    echo    - this dataset is not verified single-plane. Review before including.
    echo.
)
...
REM ============================================================
REM Pre-flight: single-plane verification (ch0_cam<CAM>.xml)
REM hs_single_plane must be "T"; warn-only - never skips
REM ============================================================
set "SP_XML=!SRC_DIR!\ch0_cam!CAM_NUM!.xml"
set "SP_FLAG=no xml"
if exist "!SP_XML!" (
    set "SP_FLAG=no line"
    find "hs_single_plane=" < "!SP_XML!" >nul
    if not errorlevel 1 (
        set "SP_FLAG=F"
        find "hs_single_plane=""T""" < "!SP_XML!" >nul
        if not errorlevel 1 set "SP_FLAG=T"
    )
)
if "!SP_FLAG!"=="T" (
    echo   Single-plane OK: hs_single_plane="T" in ch0_cam!CAM_NUM!.xml.
    echo.
) else (
    echo  [WARNING] Not verified single-plane: hs_single_plane !SP_FLAG! in ch0_cam!CAM_NUM!.xml
    echo    - proceeding anyway; manually verify results if output looks wrong.
    echo.
)
...
if "!DO_REG!"=="1" if "!SKIP_DATASET!" NEQ "1" (
    ...
    (
        echo ================================================
        echo   SPIM Processing Log - Single-Plane Variant
